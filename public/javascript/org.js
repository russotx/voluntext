...
database.ref().on('value',function(snapshot){
  ...
  for(let i in dbImage.Volunteers) {
    let vName = $('<p>');
    let vEmail = $('<p>');
    let vTotal = $('<p>');
    ...
    vName.html(dbImage.Volunteers[i].name);
    vEmail.html(dbImage.Volunteers[i].email);
    if (dbImage.Volunteers[i].totalHours != undefined) {
      vTotal.html(dbImage.Volunteers[i].totalHours);
    } else {
        vTotal.html('none');
      }
    name.append(vName);
...
function createUser () {
  ...
  let password = document.getElementById("volunteerPassword").value;
  //console.log(email);
...
function onboardVol() {
  ...
  let newVolData = fieldData.grabAll();
  // console.log(newVolData);
  let volSMSpref = isSMSopt();
...
function isSMSopt() {
  let smsBox = document.getElementById('sms-toggle');
  if (smsBox.checked) {
    return true;
  } else {
      return false;
...
function isAlpha(inputString) {
  ...
  let strNoSpaces = inputString.split(' ').join('');
  console.log(strNoSpaces);
  for (let i = 0; i < strNoSpaces.length; i++) {
    let code = strNoSpaces.charCodeAt(i);
...
function isNumeric(str) {
  ...
  let strNoDashes = str.split('-').join('');  
  for (let i = 0; i < strNoDashes.length; i++) {
    let code = strNoDashes.charCodeAt(i);
...
function newUserToDB(userData=[]) {
  let volsRef = database.ref('/Volunteers/');
  let newVolRef = volsRef.push();
  let volLink = newVolRef.toString();
  let volKey = newVolRef.key;
  let today = getToday();
  let newVol = {
    name : userData[0],
    email : userData[1],
  ...
  let fanout = {};
  let noDotEmail = emailToKey(userData[1]);
